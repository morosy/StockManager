...
ホーム画面のボード表示領域は、左右スワイプで隣接ボードへ移動できる。

重要:
- ボード表示は `HorizontalPager` ベースで構成する
- 横方向にホールドしてスライドしている間、ページは指の移動量へ追従する
- スワイプが確定したら、表示中ページに対応する `currentBoardId` を更新する
- ドロワーからボードを選択した場合も、ページャー側は同じボード位置へ同期する
- ボードごとのグリッドスクロール位置は個別に保持する
- ドロワー表示中は誤操作を避けるため、ホーム画面側の横スワイプを無効化する
- ボードが 0 件のときはページャーを表示せず、従来どおり全体の空状態を表示する
- ボードは存在するが、選択中ページ内にアイテムがない場合は、そのページ内だけ空状態を表示する
...
- チュートリアル開始時に、`ボードを追加`ステップを含む 13 枚構成か、除外した 12 枚構成かを決定する
...
- 既存ボードがある状態で開始した場合は、`ボードを追加` ステップを飛ばした 12 枚構成になる
- `ボード編集を開く` はドロワー内の `ボードを追加・編集` ボタン自体をハイライトする
- `アイテムについて` はメイン画面の現在表示中ボードの先頭アイテムをハイライトする
- `表示切り替え` はドロワー内の選択中ボード項目をハイライトする
- ドロワー内ターゲットは閉鎖開始時に登録解除し、ハイライトが閉じアニメーションへ追従しないようにしている
...
- `oss_licenses.txt` 内には開発者向けの保守メモを書かず、ユーザー向けの表示内容だけを残す
...
- ホーム画面下部は edge-to-edge で描画し、FAB 直下の 24dp 余白は維持したまま、そのさらに下は system navigation inset 分だけ可変にする
...
- `currentBoardId` とホーム画面のページ位置は常に同期している前提を崩さない
...
- ボード順変更が保持されるか
- ホーム画面で左右スワイプすると隣接ボードへ切り替わるか
- 横スワイプ中にページが指へ追従して見えるか
- ドロワー選択とスワイプ切り替えで `currentBoardId` がずれないか
